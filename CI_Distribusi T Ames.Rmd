...
```{r}
n_responden_baru <- 20
data_sam <- ames[sample(nrow(ames), n_responden_baru), ]
...
```{r}
library(dplyr)
catSale<-select(data_sam, Sale.Type,SalePrice,Sale.Condition)
catGarage<-select(data_sam, Garage.Type,Garage.Yr.Blt,Garage.Finish,Garage.Cars,Garage.Area,Garage.Qual,Garage.Cond)

```

```{r}
meancatSale <- mean(colMeans(catSale))
meancatSale
```
