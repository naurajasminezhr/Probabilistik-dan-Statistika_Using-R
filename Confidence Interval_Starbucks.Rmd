---
title: "Confidence Interval_Starbucks"
...
for(i in 1:50){
  samp <- sample(population, n) # obtain a sample of size n = 10 from the population
...
```
```{r}
population<- dat$productRate+ dat$priceRate + dat$promoRate
samp <- sample(population, 20)
sample_mean <- mean(samp)
sample_mean
```
```{r}
  mean1<- mean(dat$productRate)
  mean2<- mean(dat$priceRate)
  mean3<- mean(dat$promoRate)
  mean1
  mean2
  mean3
  mean4<- (mean1+mean2+mean3)/3
  mean4
```
```{r}
meankat1 <- mean(dat$productRate+dat$priceRate+dat$promoRate)
meankat1
```
...
ci_norm = function(samp_mean , samp_sd, n, type = "z"){
  if(type == "z"){
    ci_upper = samp_mean+(1.96*samp_sd /sqrt(n))
    ci_lower = samp_mean-(1.96*samp_sd /sqrt(n))
  }else{
    ci_upper = samp_mean+(qt(0.975,n-1)*samp_sd /sqrt(n))
    ci_lower = samp_mean-(qt(0.975,n-1)*samp_sd /sqrt(n))
  }
 res =  cbind(ci_lower,ci_upper)
 res
}

ci_norm(samp_mean , samp_sd, 30 , type ="t")
```
